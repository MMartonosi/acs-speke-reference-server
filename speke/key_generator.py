...
from oss.wrapper import acs_oss_get_secret
...
class KeyGenerator:
    ...
    def retrieve_content_id_secret(self, content_id):
        ...
        try:
            ...
            secret = acs_oss_get_secret(content_id)  # oos get key by secret_id(path - id of rhb)
    ...
    def key(self, content_id, key_id):
        ...
        kdf = PBKDF2HMAC(algorithm=hashes.SHA256(), length=derived_key_size,
                         salt=self.retrieve_content_id_secret(content_id).encode(
                             'utf-8'),
                         iterations=derived_key_iterations, backend=backend)
